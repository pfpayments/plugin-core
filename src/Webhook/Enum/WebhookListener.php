...
namespace PostFinanceCheckout\PluginCore\Webhook\Enum;

/**
 * Defines the Webhook Listeners, linking a unique ID to a technical name.
 * This acts as the single source of truth for all listener types, and it's retrieved from the postfinancecheckout backend.
 */
enum WebhookListener: int
{
    ...
    {
        return match ($this) {
            self::DELIVERY_INDICATION => \PostFinanceCheckout\PluginCore\DeliveryIndication\State::class,
            self::MANUAL_TASK => \PostFinanceCheckout\PluginCore\ManualTask\State::class,
            self::REFUND => \PostFinanceCheckout\PluginCore\Refund\State::class,
            self::TOKEN => \PostFinanceCheckout\PluginCore\Token\State::class,
            self::TOKEN_VERSION => \PostFinanceCheckout\PluginCore\Token\Version\State::class,
            self::TRANSACTION => \PostFinanceCheckout\PluginCore\Transaction\State::class,
            self::TRANSACTION_COMPLETION => \PostFinanceCheckout\PluginCore\Transaction\Completion\State::class,
            self::TRANSACTION_INVOICE => \PostFinanceCheckout\PluginCore\Transaction\Invoice\State::class,
            self::TRANSACTION_VOID => \PostFinanceCheckout\PluginCore\Transaction\Void\State::class,
            default => null,
        };
    }
}

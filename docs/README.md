...
Welcome to the documentation for the PostFinanceCheckout Plugin Core library. In this documentation, we'll guide you through everything you need to know for integrating PluginCore in your shop system.
...
Install the library, wire it to your PostFinanceCheckout Portal credentials, and learn the two conventions the rest of the documentation assumes: how failures are reported, and how the PostFinanceCheckout Portal's global lookup data works.

**You will finish with** a configured `SdkProvider` and a verified connection to the PostFinanceCheckout Portal.
...
Settling that authorization — capturing or voiding the funds, refunding all or part of it, and retrieving the invoices, packing slips and credit notes the PostFinanceCheckout Portal generates.
...
Everything that runs with no customer present: processing the webhooks the PostFinanceCheckout Portal sends you, managing your webhook subscriptions, charging saved tokens for subscriptions, and tracking outstanding merchant tasks.

...
Completions are processed **asynchronously**: `capture()` may return a completion that is still `PENDING`, with the PostFinanceCheckout Portal reporting the final outcome later via a `TransactionCompletion` webhook — see [Webhook Processor](../4-Background-Tasks/Webhook-Processor.md). The webhook payload carries the completion ID; use the gateway's read methods to verify the actual state rather than trusting the payload:

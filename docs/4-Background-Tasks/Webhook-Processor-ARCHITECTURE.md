...
1. **Resolve the Verified State:** It obtains the authoritative state (e.g., `FULFILL`) — from the webhook payload when its signature verifies, otherwise by reading it from the PostFinanceCheckout API. See [Webhook Processor](Webhook-Processor.md#the-key-idea-state-driven-not-event-driven).
...
> **Reference:** For a visual representation of the transaction states, see the [official PostFinanceCheckout Transaction Process documentation](https://checkout.postfinance.ch/en-us/doc/payment/transaction-process#_transaction_states).
...
> **Terminology:** "transaction" below means a **database** transaction in your shop, not a PostFinanceCheckout `Transaction` (the payment entity). They are unrelated, and a webhook for any entity type — `Refund`, `DeliveryIndication`, `ManualTask` — needs the same database-level atomicity.

...
This page covers retrieving the payment methods a merchant has configured in the PostFinanceCheckout Portal, and synchronizing them into your own storage.
...
The `$method->state` property is a strongly typed `PostFinanceCheckout\PluginCore\PaymentMethod\State` enum. Use `$method->state->value` when you need the string representation (e.g., for display or database storage). The valid states are: `ACTIVE`, `CREATE`, `DELETED`, `DELETING`, and `INACTIVE`.
...
`synchronize()` fetches the current methods from the PostFinanceCheckout Portal, diffs them against what your plugin already has stored, and calls the matching operation on your repository — you never write the diffing logic yourself. To use it, implement `PaymentMethodRepositoryInterface`:
...
The API returns absolute image URLs containing a `/resource/` path segment and query parameters for cache busting (e.g., `https://checkout.postfinance.ch/s/123/resource/payment/method/twint.svg?strategy=snapshot`). Use this method to extract just the clean relative file path (`payment/method/twint.svg`) for proxying, downloading, or mapping images natively.

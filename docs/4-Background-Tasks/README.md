...
Everything that runs with no customer in the browser: reacting to events the PostFinanceCheckout Portal sends you, charging saved tokens on a schedule, and surfacing work the merchant still owes.
...
The tools for programmatically managing webhooks in the PostFinanceCheckout Portal: URL creation, listener setup, and uninstallation.
...
- **[Manual Task](ManualTask.md)** — Counting the actions a merchant still has to complete in the PostFinanceCheckout Portal, so your plugin can surface a badge or reminder.

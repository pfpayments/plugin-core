...
```php
use PostFinanceCheckout\PluginCore\Token\TokenizationMode;
...
This only succeeds if the transaction supports tokenization (i.e. it was created with one of the `TokenizationMode` values above) — it throws `MissingTokenException` (in `PostFinanceCheckout\PluginCore\Token\Exception`, a subclass of `TokenException`) if the transaction has no associated token.
...
|---|---|---|
| `customerId` | Your shop's own customer reference, supplied when the token was created and echoed back by the PostFinanceCheckout Portal. | Keying: deciding which tokens belong to which customer. |

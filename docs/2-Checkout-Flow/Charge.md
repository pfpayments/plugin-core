...
A **charge flow** is an automated state machine hosted by the PostFinanceCheckout Portal. Once a transaction is confirmed, the flow decides *how* the PostFinanceCheckout Portal tries to collect the money — silently charging a saved token, generating an invoice to pay asynchronously, emailing the customer a payment link, or another method entirely, depending on how the space is configured. PluginCore does not implement this logic; it only asks the PostFinanceCheckout Portal to run the flow, and later reports what the flow produced.

## When You Don't Need `applyFlow()` (Implicit Flows)

For a standard customer-facing checkout — **Payment Page**, **Iframe**, or **Lightbox** — the PostFinanceCheckout Portal triggers the space's default charge flow **automatically** the moment the customer completes the payment widget. Your plugin never calls `applyFlow()` in this path; it only needs to read the outcome afterward (see [Reading Charge Attempts](#reading-charge-attempts) below).

This is the same "implicit confirmation" behavior described in [Checkout: Confirmation](Checkout.md#core-concepts) — confirming and charging happen together, as a single step on the PostFinanceCheckout Portal's side, with no explicit call from your code.
...
`applyFlow()` asks the PostFinanceCheckout Portal to process a transaction through its configured charge flow. Wire the service up once — typically in your plugin's DI container — and inject it wherever a charge is triggered:

```php
use PostFinanceCheckout\PluginCore\Sdk\WebServiceAPIV1\ChargeGateway;
...
Descriptor IDs and group IDs are defined by the PostFinanceCheckout Portal, not by PluginCore. Resolve them into human-readable names through [Global Data](../1-Getting-Started/GlobalData.md), or look them up in the PostFinanceCheckout Portal's label descriptor list.

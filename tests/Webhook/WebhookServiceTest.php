...
class WebhookServiceTest extends TestCase
{
    ...
    public function testInstallWebhook(): void
    {
        ...
        // Expect getUrl call
        $expectedUrl = new \PostFinanceCheckout\PluginCore\Webhook\WebhookUrl(99, $config->name, $config->url, 1);
        $this->managementGateway->expects($this->once())
            ->method('getUrl')
            ->with($spaceId, 99)
    ...
    public function testUninstallWebhook(): void
    {
        ...
        $listenerDTO = new \PostFinanceCheckout\PluginCore\Webhook\WebhookListener(
            $listenerId,
            'Test Listener',
            $entityId,
            [$stateId],
    ...
    public function testCreateWebhookListener(): void
    {
        $spaceId = 123;
        $urlId = 99;
        $entityEnum = \PostFinanceCheckout\PluginCore\Webhook\Enum\WebhookListener::TRANSACTION;
        $stateId = 'active';
        $name = 'Listener';
        $expectedId = 100;
    ...
    public function testDeleteWebhookUrlWithCascade(): void
    {
        ...
        $listener1 = new \PostFinanceCheckout\PluginCore\Webhook\WebhookListener(101, 'L1', 1, []);
        $listener2 = new \PostFinanceCheckout\PluginCore\Webhook\WebhookListener(102, 'L2', 1, []);

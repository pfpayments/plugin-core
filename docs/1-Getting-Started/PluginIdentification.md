...
Every API call can carry four headers naming the shop system and plugin behind it. The PostFinanceCheckout Portal uses them to tell integrations apart — which platform a request came from, which version of it, and which version of your plugin — which is what makes a support case or an error report traceable back to a concrete installation.
...
Both live in `PostFinanceCheckout\PluginCore\Sdk`, alongside `SdkProvider` — the metadata is part of how the SDK client is configured, not a domain of its own.

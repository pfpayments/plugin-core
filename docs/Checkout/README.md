...
```php
use PostFinanceCheckout\PluginCore\LineItem\LineItemAttribute;
use PostFinanceCheckout\PluginCore\LineItem\LineItemAttributeCollection;
...
class ShopPersistenceStrategy implements TransactionPersistenceInterface
{
    public function persist(int $transactionId): void
    {
        ...
        // $this->quote->setPostFinanceCheckoutTransactionId($transactionId)->save();
    }
}
```
...
$context->transactionId = $_SESSION['postfinancecheckout_transaction_id'] ?? null; // Load existing if any

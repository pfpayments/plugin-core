...
This directory contains the documentation for the various modules of the PostFinanceCheckout Plugin Core library.

## Running the Examples

Every runnable script under a module's `example/` directory shares the same bootstrap (`docs/examples/Common/bootstrap.php`), which requires a PostFinanceCheckout Space ID, User ID, and API Secret, set as environment variables:
...
- [Payment Method](./PaymentMethod/): Retrieving and synchronizing PostFinanceCheckout Portal configurations for available payment methods.
...
- [Webhook](./Webhook/): Guide for handling asynchronous PostFinanceCheckout Portal events (Processing and Management).
